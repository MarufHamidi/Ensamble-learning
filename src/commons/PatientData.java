...
public class PatientData {
    ...
    // resign after calculating from the classfierWeight
    // also returns the calculated value
    public double resetWeight(double classifierWeight){
        // - -
        if(isMisClassified()){
            weight = weight * 1.0 * Math.exp(classifierWeight);
        }
        // - +
        else{
            weight = weight * 1.0 * Math.exp(-classifierWeight);
